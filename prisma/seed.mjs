...
async function main() {
  ...
  await prisma.user.upsert({
    ...
    update: {},
    create: { username: "psiholog1", password: staffPassword, name: "Maria Popescu", role: "psiholog" },
  });

  await prisma.user.upsert({
    where: { username: "asistent1" },
    update: {},
    create: { username: "asistent1", password: staffPassword, name: "Ion Ionescu", role: "asistent" },
  });

  await prisma.user.upsert({
    where: { username: "farmacist1" },
    update: {},
    create: { username: "farmacist1", password: staffPassword, name: "Elena Dumitrescu", role: "farmacist" },
  });
